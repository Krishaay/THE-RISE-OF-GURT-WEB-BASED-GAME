...
export class MainScene extends Phaser.Scene{
    ...
    update(time, delta){
        ...
        let vx = 0;
        let vy = 0;
        if((this.keys.left.isDown || this.arrowKeys.left.isDown || this.mobileControls.left ) && (this.keys.right.isDown || this.arrowKeys.right.isDown || this.mobileControls.right))
        {
            if(this.vxp == 1)
            {vx = 1;}
            else
        ...
        if((this.keys.up.isDown || this.arrowKeys.up.isDown || this.mobileControls.up) && (this.keys.down.isDown || this.arrowKeys.down.isDown || this.mobileControls.down))
        {
            if(this.vyp == 1)
            {vy = 1;}
            else
        ...
        else if(this.keys.up.isDown || this.arrowKeys.up.isDown || this.mobileControls.up){
            vy = -1;
        }
        else if(this.keys.down.isDown || this.arrowKeys.down.isDown || this.mobileControls.down){
            vy = 1;
        }
        ...
        if ((this.sprintKey.isDown || this.mobileControls.sprint) && this.playerStamina > 5 && (vx !== 0 || vy !== 0)) {
            sprinting = true;
            speed = 150;
        ...
        if(goodsshop)
        {
            ...
            if(this.isMobile)
            {
                this.shopText.setText("Tap 💬 to talk");
            }
            else
            {
                this.shopText.setText("Press E to talk");
            }
        ...
        {
            const visible =
                this.isMobile && this.isNearShop;

            this.btnTalk.setVisible(visible);
            this.btnTalk.label.setVisible(visible);
        }
    ...
    openShopMenu() {
        ...
        const SHOP_DEPTH = 1000000000000010;

        this.shopUI = this.add.rectangle(520, 250, 335, 200, 0x000000, 0.8)
            .setScrollFactor(0)
            .setDepth(SHOP_DEPTH);

        this.shopTitle = this.add.text(380, 180, "Shopkeeper", {
            fontSize: "22px",
            fill: "#fff"
        }).setScrollFactor(0).setDepth(SHOP_DEPTH+1);

        this.shopBuyText = this.add.text(380, 230, "Buy Health Potion (2 gold)", {
            fontSize: "18px",
            fill: "#0f0"
        }).setScrollFactor(0).setDepth(SHOP_DEPTH+1).setInteractive();

        this.shopBuyText.on('pointerdown', () => {
            this.buyPotion();
        });

        this.shopClose = this.add.text(480, 300, "[Close]", {
            fontSize: "16px",
            fill: "#ff4444"
        }).setScrollFactor(0).setDepth(SHOP_DEPTH+1).setInteractive();
    ...
    createMobileControls() {
        ...
        const makeButton = (x, y, label) => {
            ...
            txt.setDepth(1000000000);
            btn.label = txt;

            return btn;
        };

        const makeButton2 = (x, y, label) => {
            const btn = this.add.circle(
                x,
                y,
                40,
        ...
        this.btnLeft  = makeButton(baseX - 60, baseY, '◀');
        this.btnRight = makeButton(baseX + 60, baseY, '▶');
        this.btnUp    = makeButton(baseX, baseY - 60, '▲');
        this.btnDown  = makeButton(baseX, baseY + 60, '▼');

        this.btnAttack = makeButton2(w - 230, h - 150, '⚔');
        this.btnSprint = makeButton2(w - 320, h - 150, '⚡');
        this.btnPotion = makeButton(w - 220, h - 400, '🧪');

        this.btnTalk = makeButton(
            w - 220,
            h - 300,
            '💬'
        );
        this.btnTalk.setVisible(false);
        this.btnTalk.label.setVisible(false);
        ...
        bindHold(this.btnUp, 'up');
        bindHold(this.btnDown, 'down');
        bindHold(this.btnSprint, 'sprint');

        this.btnAttack.on('pointerdown', () => {
            this.attackMelee();
        });

        this.btnPotion.on('pointerdown', () => {
            this.usePotion();
        });

        this.btnTalk.on('pointerdown', () => {
            if(this.isNearShop)
            {
                this.openShopMenu();
            }
